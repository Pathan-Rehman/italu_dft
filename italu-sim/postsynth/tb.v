...
module tb;
    ...
    initial begin
        ...
        i = 0;
        while (!dut.bist_done && i < 700) begin
            @(posedge clk);
            i = i + 1;
        end

        tests = tests + 1;
        if (dut.bist_done)          $display("PASS BIST DONE (%0d cycles)", i);
        else begin errors = errors + 1; $display("FAIL BIST timeout"); end

        @(posedge clk);
        #1;
        tests = tests + 1;
        if (dut.bist_done)          $display("PASS BIST DONE sticky");
        else begin errors = errors + 1; $display("FAIL BIST DONE not sticky"); end

        tests = tests + 1;
        if (dut.test_pass)          $display("PASS BIST PASS=1");
        else begin errors = errors + 1; $display("FAIL BIST PASS=0"); end

        tests = tests + 1;
        if (dut.misr === 8'h93)     $display("PASS MISR signature 93");
        else begin errors = errors + 1; $display("FAIL MISR=%02h", dut.misr); end
        ...
        tests = tests + 1;
        if (!dut.test_pass)         $display("PASS FAULTED BIST detected");
        else begin errors = errors + 1; $display("FAIL FAULTED BIST passed"); end

        tests = tests + 1;
        if (dut.fault_counter >= 1) $display("PASS FAULT COUNTER=%0d", dut.fault_counter);
